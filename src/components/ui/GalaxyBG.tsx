...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    vec2 mouseOffset =
      mouseNorm * 0.1 * uMouseActiveFactor;
...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    const update = (time: number) => {
      ...
      smoothMouse.current.x +=
        (targetMouse.current.x - smoothMouse.current.x)
        * lerp

      smoothMouse.current.y +=
        (targetMouse.current.y - smoothMouse.current.y)
        * lerp

      smoothActive.current +=
        (targetActive.current - smoothActive.current)
        * lerp
